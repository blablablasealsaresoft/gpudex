# BILL GATES ULTIMATE 502 FIX - CLEAN VERSION (NO UNICODE ISSUES)
...
Write-Host ""
Write-Host "STEP 2: Removing old containers completely..." -ForegroundColor Yellow
...
try {
    ...
    $contentPreview = $health.Content.Substring(0, [Math]::Min(50, $health.Content.Length))
    Write-Host "SUCCESS Health endpoint: $($health.StatusCode) - $contentPreview" -ForegroundColor Green
} catch {
...
try {
    ...
    Write-Host "SUCCESS Main page: $($main.StatusCode)" -ForegroundColor Green
    Write-Host "  Content length: $($main.Content.Length) characters" -ForegroundColor White
    
    if ($main.Headers.ContainsKey('Content-Type')) {
        Write-Host "  Content type: $($main.Headers.'Content-Type')" -ForegroundColor White
    }
    
    # Check for specific content
    if ($main.Content -match "GPUDx") {
        Write-Host "  SUCCESS Contains GPUDx branding" -ForegroundColor Green
    }
    if ($main.Content -match "StakingDashboard") {
        Write-Host "  SUCCESS Contains StakingDashboard component" -ForegroundColor Green
    }
    if ($main.Content -match "AchievementSystem") {
        Write-Host "  SUCCESS Contains AchievementSystem component" -ForegroundColor Green
    }
    if ($main.Content -match "InfluencerDashboard") {
        Write-Host "  SUCCESS Contains InfluencerDashboard component" -ForegroundColor Green
    }
} catch {
...
try {
    ...
    Write-Host "SUCCESS Favicon: $($favicon.StatusCode)" -ForegroundColor Green
} catch {
    Write-Host "WARNING Favicon: $($_.Exception.Message)" -ForegroundColor Yellow
}
...
foreach ($component in $components) {
    try {
        ...
        Write-Host "  SUCCESS $component : $($comp.StatusCode)" -ForegroundColor Green
    } catch {
        Write-Host "  FAILED $component : Failed" -ForegroundColor Red
    }
}
...
try {
    ...
    Write-Host "SUCCESS Nginx load balancer: $($nginx.StatusCode)" -ForegroundColor Green
} catch {
    Write-Host "FAILED Nginx load balancer: $($_.Exception.Message)" -ForegroundColor Red
}
...
try {
    ...
    foreach ($container in $finalStatus) {
        if ($container.Service -eq "frontend") {
            if ($container.State -eq "running") {
                ...
                Write-Host "SUCCESS Frontend container: RUNNING" -ForegroundColor Green
            } else {
                Write-Host "FAILED Frontend container: $($container.State)" -ForegroundColor Red
            }
        }
        if ($container.Service -eq "nginx") {
            if ($container.State -eq "running") {
                $nginxRunning = $true
                Write-Host "SUCCESS Nginx container: RUNNING" -ForegroundColor Green
            } else {
                Write-Host "FAILED Nginx container: $($container.State)" -ForegroundColor Red
            }
        }
    }
    ...
    if ($frontendHealthy -and $nginxRunning) {
        ...
        Write-Host "  4-Tier Staking (Bronze to Diamond)" -ForegroundColor White
        ...
        Write-Host "Some issues may remain. Check container logs for details." -ForegroundColor Yellow
    ...
    Write-Host "Error during final status check: $($_.Exception.Message)" -ForegroundColor Red
    Write-Host "Try manual commands above" -ForegroundColor Yellow
} 